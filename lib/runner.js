...
const messageHandlers = {
  get(args, data) {
    ...
    if (args.preserveCookies) {
      performGet(args, data);
    } else {
      // removing localstorage removes cookies as well
  ...
  js(args, data) {
    mainWindow.webContents.executeJavaScript(args, true)
      .then((result) => {
        process.send(result);
      });
  },
  ...
  getNamedCookie(name) {
    session.defaultSession.cookies.get({}).then((error, cookies) => {
      if (error) {
        process.send(wrapElectronResult(null, error));
        return;
      }
